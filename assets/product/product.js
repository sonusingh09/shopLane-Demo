$(document).ready(function() {
    ...
    $("#btn-add-to-cart").click(function() {
        ...
        var productList = window.localStorage.getItem('selected-product');
        ...
        for(var i=0; i < productList.length; i++) {
            if(parseInt(productList[i].id) == parseInt(selectedProductId.id)) {
                foundAtPos = i;
            }
        }
        ...
        if(foundAtPos > -1) {
            ...
            window.localStorage.setItem('selected-product', JSON.stringify(productList));
        } else {
            selectedProductId.count = 1;
            productList.push(selectedProductId);
            console.log(productList);
            window.localStorage.setItem('selected-product', JSON.stringify(productList));
        }
